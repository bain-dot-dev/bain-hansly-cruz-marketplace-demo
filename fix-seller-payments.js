...
async function fixSellerPaymentSetup() {
  ...
  try {
    ...
    console.log(
      `Found ${userAccounts?.length || 0} accounts for user ${userId}:`
    );
    userAccounts?.forEach((acc, i) => {
      console.log(
        `${i + 1}. ${acc.stripe_account_id} - Created: ${
          acc.created_at
        }, Charges: ${acc.charges_enabled}`
      );
    });
    ...
    const accountsToDelete = userAccounts.filter(
      (acc) => acc.id !== accountToKeep.id
    );
    ...
    if (accountsToDelete.length > 0) {
      ...
      for (const account of accountsToDelete) {
        ...
        if (deleteError) {
          console.error(
            `   ❌ Failed to delete ${account.stripe_account_id}:`,
            deleteError
          );
        } else {
    ...
    if (accountToKeep.charges_enabled) {
      ...
      console.log(
        '   User should visit /profile and complete the "Complete Setup" process'
      );
    }
  } catch (e) {
